...
| (14),(15) | attribute, data-attribute set | partial M0 | arithmetic/string/`ALIGNED` subset; `POINTER` (an address value) — `DECLARE P POINTER;` a pointer variable whose value is the null pointer (`null()`), the address of a variable (`addr(x)`), or another pointer; pointers assign between each other and compare with `=`/`^=` against the null pointer and other pointers, while mixing a pointer with an arithmetic value or an ordered pointer comparison is diagnosed (`pointer.pli`, `bad_pointer.pli`, ADR-063); `BASED`/`CONTROLLED`/`AREA`/`OFFSET` still M3 |
...
| (104),(105) | `GET`/`PUT` and options | partial M2 | `PUT [SKIP] [PAGE] LIST`; `GET [SKIP] LIST` list-directed input reading scalar values from SYSIN (`get.pli`, `bad_get.pli`, ADR-066); `STRING ( reference )` routes list-directed output into / input from a NONVARYING character variable (`string.pli`, `bad_string.pli`, ADR-067); `FILE ( f )` routes output/input through an opened named file (`driver/file`, `bad_file.pli`, ADR-068); `EDIT` edit-directed transmission is served (see (106)-(111), ADR-069); `DATA`/`COPY`/`LINE` → M5 |
...
| (123) | `emitExpr` (`Call`) | per-builtin handlers: SUBSTR, INDEX, ABS, LENGTH, TRUNC, PRECISION, MIN, MAX, MOD, MULTIPLY, DIVIDE, ROUND, REPEAT, VERIFY, TRANSLATE, HIGH, LOW, DATE, TIME, LBOUND, HBOUND, DIM, SUM, PROD, ANY, ALL, NULL, ADDR |
