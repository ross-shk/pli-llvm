...
| (12),(13) | dimension attribute, bound pairs | partial M2 | fixed-size constant-bounds arrays, single- and multi-axis, row-major layout (`A(m,n)`/`A(lb:ub,...)`), scalar elements (`array.pli`, `array2d.pli`); single-axis dynamic-bounds AUTOMATIC arrays `A(n)`/`A(lb:n)` with a runtime upper bound, extent fixed at block entry, runtime `SUBSCRIPTRANGE` and runtime `LBOUND`/`HBOUND` (`dynamic_array.pli`, ADR-050); a dynamic lower bound `A(lb:ub)` on a local AUTOMATIC single-axis array — both bounds evaluated at entry, extent, `SUBSCRIPTRANGE`, `LBOUND`/`HBOUND`/`DIM`, and array reductions use the live bounds (`dyn_lower.pli`, `bad_dyn_lower.pli`, ADR-058); a fixed or dynamic array passed to a procedure whose parameter is a dynamic array `x(k)` — the callee sizes the view from the bound argument at entry and bounds-checks and reduces against the live extent, and writes through the parameter are visible in the caller (`dyn_param.pli`, ADR-054); `*` adjustable-extent array parameters `x(*)` — a single-axis parameter whose extent comes from the caller's actual array (fixed, dynamic-bound, or multi-axis) at call time, passed as a hidden extent argument; bounds-checked and reduced against the live extent, by-reference writes visible in the caller (`star_param.pli`, ADR-055); multi-axis dynamic, `INITIAL` on a dynamic array, dynamic structure members, a block-scope `*`, and a dynamic lower bound on a parameter → diag (`bad_star_param.pli`, `bad_dyn_param.pli`, `bad_dynamic_array*.pli`, `bad_dyn_lower.pli`); cross-sections → M2 |
