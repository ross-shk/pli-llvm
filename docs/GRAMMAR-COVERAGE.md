...
| (104),(105) | `GET`/`PUT` and options | partial M2 | `PUT [SKIP] [PAGE] LIST`; `GET [SKIP] LIST` list-directed input reading scalar values from SYSIN (`get.pli`, `bad_get.pli`, ADR-066); `STRING ( reference )` routes list-directed output into / input from a NONVARYING character variable (`string.pli`, `bad_string.pli`, ADR-067); `FILE ( f )` routes output/input through an opened named file (`driver/file`, `bad_file.pli`, ADR-068); `EDIT`/`DATA`/`COPY`/`LINE` → M5 |
