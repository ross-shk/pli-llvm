...
| (134) | `isub` (`integer SUB`) | partial M2 | iSUB dummy (`1SUB`, `2SUB`, lexed with no blanks) inside a `DEFINED` subscript base: `DECLARE R(n) DEFINED X(c, 1SUB)` makes R a live 1-D overlay of one axis of X, and `DEFINED X(c, c)` overlays a single element — writes through the overlay are visible in the base and vice versa (`isub_defined.pli`, ADR-048); more than one iSUB, a scalar item on an iSUB base, a non-constant subscript, or an out-of-bounds constant is diagnosed (`bad_isub_defined.pli`); general iSUB index arithmetic (`X(2*1SUB)`) and multi-axis iSUB → M2 |
