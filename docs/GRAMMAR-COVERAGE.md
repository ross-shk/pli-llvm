...
| (12),(13) | dimension attribute, bound pairs | partial M2 | fixed-size constant-bounds arrays, single- and multi-axis, row-major layout (`A(m,n)`/`A(lb:ub,...)`), scalar elements (`array.pli`, `array2d.pli`); single-axis dynamic-bounds AUTOMATIC arrays `A(n)`/`A(lb:n)` with a runtime upper bound, extent fixed at block entry, runtime `SUBSCRIPTRANGE` and runtime `LBOUND`/`HBOUND` (`dynamic_array.pli`, ADR-050); a fixed or dynamic array passed to a procedure whose parameter is a dynamic array `x(k)` — the callee sizes the view from the bound argument at entry and bounds-checks and reduces against the live extent, and writes through the parameter are visible in the caller (`dyn_param.pli`, ADR-054); dynamic lower bound, multi-axis dynamic, `*` extents, `INITIAL` on a dynamic array, dynamic structure members → diag (`bad_dyn_param.pli`, `bad_dynamic_array*.pli`); cross-sections → M2 |
...
| (34)–(38) | `ENTRY`, `RETURNS`, descriptors, `USES`/`SETS` | partial M0 → M2 | ADR-021: scalar C entry/calls served; fixed-size arrays and structures passed by reference to PL/I procedures (`array_param.pli`, `struct.pli`); dynamic (runtime-extent) arrays passed to procedures via a by-reference bound argument sized at callee entry (`dyn_param.pli`, ADR-054); dope-vector descriptors for `*` extents → M2 |
